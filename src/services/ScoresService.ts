...
class ScoresService {
  async getScoreBySession(id: number): Promise<Scores> {
    ...
    const result = await scoresRepository.findOne({
  ...
  async getBestScoreBySong(songId: string): Promise<IMaxSessionScore[]> {
    const result = (await ScoresRepository.findBestScoreBySong(
      songId,
    )) as IMaxSessionScore[];
  ...
  async getHistoryBySong(
    userId: number,
    songId: string,
  ): Promise<IMaxSessionScore[]> {
    const result = (await ScoresRepository.getHistoryBySong(
      userId,
      songId,
    )) as IMaxSessionScore[];
